...
where $Y$ is a vector with phenotypes, $X$ the design matrix for the cross structure, $\beta$ a vector of fixed cross intercepts, and the residual term $\varepsilon$ has a cross-specific variance covariance structure  $\oplus_{k = 1}^{C} I_{{n_{k} }}$ in which $\sigma_{{\varepsilon_{k} }}^{2} $ is the residual variance of the $k^{th}$ cross with family size $n_{k}$. 
...
where $M_{q}$ is the design matrix containing the expected number of parental alleles, $a_{q}$ is the vector of random parental effects with variance covariance structure $I_{P} \sigma_{q}^{2}$, where $\sigma_{q}^{2}$ is the genetic variance of the QTL effect.

From these two model a p-value is then estimated for each of the putative QTLs using a likelihood ratio test (LRT).

For CIM the first steps are the same. However after the first round of scanning the QTL with the lowest $-10log(p)$ value is added as a cofactor in the model and a new round of scanning is done. This is repeated until no new QTLs are found with a $-10log(p)$ value above the predefined `threshold`, or until the predefined maximum number of cofactors (`maxCofactors`) is added to the model. In a region of width `QTLwindow` around a cofactor no new cofactors can be detected.

#### Example maize

Going back to our maize example we can first perform SIM.

```{r loadMaize, include=FALSE}
## Use saved data to speed up vignette building.
#usethis::use_data(maizeSIM, maizeCIM, internal = TRUE, overwrite = TRUE)
load(system.file("R/sysdata.rda", package = "statgenMPP"))
```
