...
plotQTLscanOrig <- function(data,
  ...
  col_chr <- c("black", "red", "green", "blue", "cyan1", "purple", "gold",
               "brown", "chartreuse", "black", "black", "black")
  col <- col_chr[chromF]
  max_y <- 5.0 * ceiling(max(data$minlog10p) / 5.0)
  plot(x = x, y = data$minlog10p, ylim = c(0, max_y), col = "blue", yaxs = "i",
       cex = 0.55, type="l", axes = FALSE, xpd = FALSE, pch = 16,
       ylab = "-log10(P)", xlab = "Chromosomes", main = title)
  # defines the end points of the chromosomes:
  abline(v = ver_line, col = "red", lwd = 2.0, lty = 3)
  abline(h = threshold, col = "red",lwd = 0.7)
  if (nchr == 1) {
    axis(1, at = xtics[1], labels = nchr)
  } else {
    axis(1, at = xtics, labels = 1:nchr)
  }
  axis(2)
  box()
}
